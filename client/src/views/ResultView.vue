<template>
  <div class="page" id="result-page" style="padding-top: 40px">
    <div class="result-card" id="result-capture">
      ...
      >
        ...
        <span style="font-size: 0.8rem"
          >Anda sedang melihat hasil sebagai Admin.</span
        >
      </div>
      ...
      <div
        style="color: var(--text-muted); font-size: 0.8rem; margin-bottom: 12px"
      >
      ...
      <div
        v-if="!isPreview"
        style="color: var(--text-muted); font-size: 0.8rem; margin-bottom: 16px"
      >
      ...
      <div v-if="spinHistory.length" style="margin-bottom: 16px">
        ...
        >
          ...
          >
            ...
            <div
              v-if="s.is_free_spin"
              style="color: var(--accent-green); font-size: 0.7rem"
            >
              🎁
            </div>
            <div
              v-else-if="s.is_rare"
              style="color: var(--accent-gold); font-size: 0.7rem"
            >
              ⭐
            </div>
            <div
              v-else-if="s.is_zonk"
              style="color: #ff6b6b; font-size: 0.7rem"
            >
              💀
            </div>
          </div>
    ...
    <div
      ...
      style="max-width: 480px; width: 100%; justify-content: center"
    >
      <button
        v-if="!isPreview"
        class="btn btn-wa"
        @click="sendWa"
        style="padding: 16px 24px; font-size: 1.1rem; width: 100%"
      >
...
const isPreview = computed(
  () => route.query.preview === "true" && store.isAdmin,
);
...
function sendWa() {
  ...
  const msg = `\uD83C\uDF8A *KLAIM HADIAH THR SPIN HARI RAYA* \uD83C\uDF8A\n\nHalo Admin, saya telah menyelesaikan permainan dan ingin mengklaim hadiah THR saya. Berikut adalah rinciannya:\n\n\uD83D\uDC64 *Detail Pemain:*\n- Nama: ${name}\n- E-Wallet: ${ewallet}\n\n\uD83D\uDCB0 *Rincian Hadiah:*\n- Total Hadiah: *${prize}*\n- Waktu Selesai: ${new Date().toLocaleString("id-ID", { timeZone: "Asia/Jakarta", dateStyle: "long", timeStyle: "short" })} WIB\n\n\uD83D\uDD17 *Link Hasil:*\n${window.location.origin}${route.path}?preview=true\n\nMohon bantuannya untuk memproses pencairan hadiah. Terima kasih banyak! \uD83D\uDE4F`;
  let cleanWa = adminWa.replace(/\D/g, "");
